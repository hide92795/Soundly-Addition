...
public abstract class ManagedSound {
	...
	public float mixedPitch() {
		return (getMix() != null ? getMix().getPitch() : 1f) * getPitch()
	...
	public float mixedX() {
		return (getMix() != null ? getMix().getXAdjustment() : 0f) + getX()
				+ (getParent() != null ? getParent().mixedX() : 0f);
	}

	public float mixedY() {
		return (getMix() != null ? getMix().getYAdjustment() : 0f) + getY()
				+ (getParent() != null ? getParent().mixedY() : 0f);
	}

	public float mixedDepth() {
		return (getMix() != null ? getMix().getDepthAdjustment() : 0f) + getDepth()
				+ (getParent() != null ? getParent().mixedDepth() : 0f);
	}
	...
	// TODO: cleanup distance checking, reduce calls to calculateDistance(), add
	// withinEarshot() EVENT
...
// HUGE ISSUE: if SimpleSound is placed in multiple sources with different
// priorities,
// how does the manager know which priority to check against?
// SOLUTION: add method play(....) which allows for a one-off, otherwise you
// need to use XSound
//
