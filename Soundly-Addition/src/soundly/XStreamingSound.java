 ...
 * you must use setDefaultIndex or reset (which will stop playback and create a
 * new
...
public class XStreamingSound extends XSound {
	...
	/** Only OGG streams and MOD sounds are currently accepted. */
	public XStreamingSound(InputStream stream, String format, String description) throws SlickException {
		super(new AudioData(), description);
		SoundStore.get().init();
		try {
			if (format.toLowerCase().equals("ogg")) {
				...
				if (SoundStore.get().isDeferredLoading())
					Log.warn("Deferred loading for MOD is not supported; loading immediately");
				modPlayer = new OpenALMODPlayer();
				module = OpenALMODPlayer.loadModule(stream);
			} else {
				throw new SlickException("not a valid stream format, must be: .xm, .mod or .ogg");
			}
		} catch (IOException e) {
			Log.error(e);
			throw new SlickException("Failed to load sound: " + stream.toString(), e);
		}
	}
	...
	public XStreamingSound(InputStream stream, String format) throws SlickException {
		this(stream, format, null);
	}
