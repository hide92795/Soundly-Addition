...
public class Soundly {
	 ...
	 * 100% means "not at center". If either sound has no radius, 0f is
	 * returned.
	 */
	/**
	 * @param sound
	...
	// if not specified, the default proximity radius will be used. otherwise
	// this will be the
	// limit at which distance priority is taken into account
	// to disable distance priority altogether and rely only on LEVEL and
	// PRIORITY,
	...
	public void init(int maxSources) {
		...
		listenerPos = BufferUtils.createFloatBuffer(3).put(new float[] { listenerX, listenerDepth, listenerY });
		listenerPos.flip();
		...
		// listenerOri = BufferUtils.createFloatBuffer(6).put(new float[] {
	...
	public int findBestSourceFor(ManagedSound sound, int startIndex, int endIndex) {
		if (startIndex < 0 || endIndex > sources.length)
			throw new IllegalArgumentException("invalid start/end indices for findBestSource " + startIndex + " "
					+ endIndex);
		...
		for (int i = startIndex; i < endIndex; i++) {
			...
			if (oSound == null) { // if there are no XSounds assigned to the
									// source
				return i;
			} else if (oSound.isReplaceable()) { // if the sound is being
													// managed...
				// if the incoming sound is of greater or equal level to the
				// other
				...
				if (sound.getLayer() >= oSound.getLayer() && oSound.getLayer() <= lowestLevel) {
					...
					// if the incoming sound has a greater or equal priority, we
					// may make a switch
					// but if we've already seen a lower priority than this,
					// just ignore
					if (sound.getPriority() >= oSound.getPriority() && oSound.getPriority() <= lowestPriority) {
	...
	public void setListenerPosition(float x, float y, float depth) {
		...
		if (oldX != x || oldY != y || oldZ != depth) {
			...
			// TODO: make float buffer changing more consistent between this and
			// sourcePos
			AL10.alListener(AL10.AL_POSITION, listenerPos);
			distanceChanged();
		}
	...
	public float distanceChanged() {
		...
		for (int i = getMusicTrackCount(); i < sources.length; i++) {
			// any static sounds that are playing should be moved with the
			// listener...
			// notify "positionChanged" which will recalculate with new listener
			// pos
			if (sources[i] != null) {
	 ...
	 * getSourceVolume to be the same as what you earlier set with
	 * setSourceVolume.
	 *
	 * @param source
	 * @param gain
	...
	public int queueAtIndex(int sourceIndex, XSound sound) {
		...
		if (SoundStore.get().soundWorks() && SoundStore.get().soundsOn()) {
			// first check to see if we've globally enabled proximity
			// checking...
			/*
			 * if (isProximityEnabled()) {
			 * //if so, do another check for the sound...
			 * //if sound's proximity is enabled, then disable the sound as soon
			 * as it's
			 * //outside of range.
			...
			// if we are to only play once, and the last source exists and is
			// playing...
	...
	public void update(int delta) {
		...
		for (int sourceIndex = 0; sourceIndex < queue.length; sourceIndex++) {
			...
			if (sound != null) {
				...
				// tell the sound to update its own sources (and that we are
				// initializing!)
	...
	// option B: store one of the lastIndices (but that doesn't cover ALL, just
	// the last!)
	...
	// when you queue it, it plays the sound at another source even if already
	// playing
	 ...
	 *
	 * @param soundIndexStart
	 *            the soundIndexStart to set
	 */
