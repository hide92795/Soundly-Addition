...
public class XOpenALStreamPlayer {
	...
	public XOpenALStreamPlayer(int source, InputStream stream) {
		this.source = source;
		this.stream = stream;
	...
	private void initStreams() throws IOException {
		...
		if (url != null) {
			...
			audio = new OggInputStream(stream);
		}
		...
		try {
			...
			if (count != -1) {
				...
				try {
					...
					Log.error(
							"Failed to loop buffer: " + bufferId + " " + format + " " + count + " " + audio.getRate(),
							e);
					return false;
				}
			} else {
